...
void incDecIfCount(bool inc) {
	if (inc) {
		if (printIfCount) writefln("incrementing if count, %s -> %s", ifStatementCount, ifStatementCount + 1);
		++ifStatementCount;
	} else {
		if (printIfCount) writefln("decrementing if count, %s -> %s", ifStatementCount, ifStatementCount - 1);
		--ifStatementCount;
	}
}
...
void compileTerm() {
	...
	if (tokens[next].type == "integerConstant") {
		...
		if (isTerminal(tokens[next+1], "[")) { // check if there's a [
			...
			//at this point, the result of the expression will be on the top of the stack.
...
void compileWhileStatement() {
	...
	++indentation;
	int thisCount = whileCount++;
	writeXML(demand("while"));
	writeVM(format("label WHILE_EXP%d", thisCount));
	writeXML(demand("("));
	compileExpression();
	writeXML(demand(")"));
	writeVM("not");
	writeVM(format("if-goto WHILE_END%d", thisCount));
	writeXML(demand("{"));
	compileStatements();
	writeXML(demand("}"));
	writeVM(format("goto WHILE_EXP%d", thisCount));
	writeVM(format("label WHILE_END%d", thisCount));
	--indentation;
	writeIndented("</whileStatement>\r\n");
}
...
void compileLetStatement() {
	...
	if (isTerminal(tokens[next], "[")) {
		...
		compileExpression();
		writeXML(demand("]"));
		writeVM(["push " ~ destVM, "add"]);
		destVM = "that 0";
	}
	writeXML(demand("="));
	compileExpression();
	writeXML(demand(";"));
	if (isArray) writeVM(["pop temp 0", "pop pointer 1", "push temp 0"]);
	writeVM("pop " ~ destVM);
	--indentation;
	writeIndented("</letStatement>\r\n");
}
...
void compileIfStatement() {
	...
	++indentation;
	int thisCount = ifStatementCount++;
	writeXML(demand("if"));
	writeXML(demand("("));
	compileExpression();
	writeXML(demand(")"));

	writeVM(format("if-goto IF_TRUE%d", thisCount));
	writeVM(format("goto IF_FALSE%d", thisCount));
	writeVM(format("label IF_TRUE%d", thisCount));
	...
	if (isTerminal(tokens[next], "else")) {
		writeVM(format("goto IF_END%d", thisCount));
	}
	writeVM(format("label IF_FALSE%d", thisCount));
	...
	if (isTerminal(tokens[next], "else")) {
		...
		compileStatements();
		writeXML(demand("}"));
		writeVM(format("label IF_END%d", thisCount));
	}
