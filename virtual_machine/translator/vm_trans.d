...
string preamble_code() {
	string init_code = "//Initialization:\n";
	init_code ~= "@256\nD=A\n@SP\nM=D\n";
	init_code ~= compile_call("call Sys.init 0");
	return init_code;
}
...
string compile_label(string op) {
	...
	return format(label_template, label);
}
...
string compile_goto(string op) {
	...
	if (inst == "goto")
		...
		return format(if_goto_template, dest);
}
...
string compile_call(string op) {
	...
	//if (num_args==0) output ~= format(push_const_template, 0);
	//push return address
	output ~= format(push_const_template, return_address);
	...
	output ~= "@LCL\nD=M\n" ~ push_tail_template ~
			  ...
			  "@THAT\nD=M\n" ~ push_tail_template;
	//ARG = SP - num_args - 5
	//int shift = (num_args>0) ? num_args : 1;
	output ~= format("@SP\nD=M\n@%s\nD=D-A\n@5\nD=D-A\n@ARG\nM=D\n", num_args);
	//LCL = SP
	output ~= "@SP\nD=M\n@LCL\nM=D\n";
	// goto function
...
string compile_return() {
	...
	output ~= "@SP\nAM=M-1\nD=M\n@ARG\nA=M\nM=D\n"; // *(ARG) = pop()
	output ~= "@ARG\nD=M+1\n@SP\nM=D\n"; // SP = ARG + 1
	foreach (ptr; ["THAT", "THIS", "ARG", "LCL"]) //set environment pointers
		output ~= format ("@FRAME\nAM=M-1\nD=M\n@%s\nM=D\n", ptr);
	output ~= "@RET\nA=M\n0;JMP\n"; //goto ret
	return output;
}
...
void main(string args[]) {
	...
	if (args.length < 2) {
		usage:
		writefln("usage: %s <filename(s)> [-o <outputfilename>] [-noinit]", args[0]);
		return;
	}

	int[] indices_to_skip;
	string outputfname;
	bool output_file_specified = false, noinit = false;
	...
	if (!noinit) output.put(preamble_code());
	for (int i=1; i<args.length; ++i) {
		if (!canFind(indices_to_skip, i)) {
			string inputfname = args[i];
			writef("Compiling %s... ", inputfname);
